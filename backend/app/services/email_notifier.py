...
class EmailNotifier:
    ...
    def send_violation(self, camera_name: str, violation_type: str, event_id: int) -> bool:
        ...
        message["Subject"] = f"[PPE Guard] {violation_type} at {camera_name}"
        message["From"] = settings.SMTP_FROM_EMAIL
        message["To"] = ", ".join(settings.alert_recipients_list)
        message.set_content(
            f"A PPE violation was confirmed.\n\n"
            f"Camera: {camera_name}\nViolation: {violation_type}\nEvent ID: {event_id}\n\n"
            "Open the PPE Guard dashboard to review and acknowledge the evidence."
        )
    ...
    def send_password_reset(self, recipient: str, token: str) -> bool:
        if not settings.SMTP_HOST or not settings.SMTP_FROM_EMAIL:
            return False
        message = EmailMessage()
        message["Subject"] = "PPE Guard password reset code"
        message["From"] = settings.SMTP_FROM_EMAIL
        message["To"] = recipient
        message.set_content(
            "A password reset was requested for your PPE Guard account.\n\n"
