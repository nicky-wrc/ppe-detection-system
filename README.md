...
| `EVENT_COOLDOWN_SECONDS`       | `30`                 | ช่วงพักก่อนสร้างเหตุการณ์ซ้ำ                   |
...
- ปรับค่าบัญชีแล้วกด **บันทึกค่าบัญชี** ค่าจะถูกเก็บในฐานข้อมูลและใช้กับการตรวจจับถัดไป ไม่ต้องเริ่มกล้องใหม่ ผลตรวจย้อนหลังไม่ถูกประมวลผลใหม่
- กล้องฝั่ง Backend ใช้ค่าของเจ้าของกล้อง ส่วนกล้องผ่านเบราว์เซอร์และการอัปโหลดใช้ค่าของบัญชีผู้ส่งงาน ไม่ได้เปลี่ยนค่าของทุกบัญชีในระบบ
- **ความมั่นใจขั้นต่ำในการตรวจพบคน**: 10–90% ลดค่าเพื่อรับผลที่มั่นใจน้อยลง แต่อาจตรวจผิดเพิ่มขึ้น
- **ความไวในการตรวจพบ PPE**: 0–100% เพิ่มค่าเพื่อลดเกณฑ์ความมั่นใจ PPE หลัก (`max(0.10, 0.45 - sensitivity × 0.0035)`) ไม่ใช่การเพิ่มความเข้มงวดในการแจ้งการฝ่าฝืน ตัวอย่าง 60% ใช้เกณฑ์หลัก 24%; person-crop refinement ยังมีเกณฑ์ย่อยของ runtime ตามเดิม
- กฎ PPE ส่วนบุคคลใช้เมื่อไม่มีโซนที่ active มากำหนดแทน ปิดรายการใดจะไม่ตรวจการขาด PPE ชนิดนั้น ปิดทั้งสองรายการยังนับคน แต่ไม่รายงานการฝ่าฝืน PPE
- **กฎโซน** เป็นกฎร่วมที่ Administrator เท่านั้นแก้ได้ กดแล้วบันทึกทันที แยกจากปุ่มบันทึกค่าบัญชี และมีลำดับความสำคัญเหนือกฎส่วนบุคคล
- **เสียงเตือน** ใช้กับบัญชีที่กำลังเปิดเว็บ ปิดแล้วยังคงมีข้อความแจ้งเตือน การบันทึกซิงก์กับแท็บอื่นใน browser/origin เดียวกัน และอ่านค่าใหม่เมื่อกลับมา focus หน้าเว็บ Browser อาจบล็อกเสียงจนกว่าจะมีการคลิกหน้าเว็บ
- **บันทึกหลักฐานจากกล้องฝั่ง Backend**: ปิดแล้วไม่บันทึกภาพ/คลิปใหม่จากกล้องของบัญชีนั้น แต่ยังเก็บเหตุการณ์และสถิติ ทิ้งเฉพาะเฟรมที่ค้างในหน่วยความจำ ไม่ลบหลักฐานเดิม ตัวเลือกนี้ยังไม่ครอบคลุม upload image/video หรือ browser-camera fallback ที่บันทึกผ่าน upload API
- ข้อมูลโมเดลอ่านจาก `/api/v1/models/active` แสดง version และการมีอยู่ของไฟล์ตามการตั้งค่า ไม่ได้ยืนยันว่า inference/GPU/กล้องทำงานสำเร็จ
- หน้า Settings แสดงรายการที่ยังไม่บันทึก มีปุ่มยกเลิกการแก้ไข และเตือนเมื่อปิด/รีโหลดแท็บที่มีค่าค้าง ควรบันทึกก่อนเปลี่ยนเมนูภายในเว็บ

หลังอัปเดตโค้ด Settings ให้เริ่ม Backend ใหม่หนึ่งครั้งและรีโหลด Frontend จากนั้นการปรับค่าระหว่างใช้งานไม่ต้องรีสตาร์ตกล้อง ห้ามเปิด cleanup กลับหรือคัดลอก `.env.example` ทับ `.env` เดิมในการทดสอบนี้
...
- เหตุการณ์ซ้ำมี cooldown 30 วินาที
